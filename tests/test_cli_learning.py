...
def finding_id(tmp_path, rule_id) -> str:
    payload = json.loads(
...
def review(tmp_path, store, rule_id, decision, capsys) -> str:
    fid = finding_id(tmp_path, rule_id)
...
def test_demo_step_4_full_flow(tmp_path, capsys):
    ...
    run_seeded(tmp_path, store)
    capsys.readouterr()

    out = review(tmp_path, store, "RULE-LOAN-BAL-001", "confirmed", capsys)
    assert "FM-001 probability: 0.7 -> 0.727" in out
    assert "confirmed 0 -> 1" in out
    assert "draft 1.0.1 pending" in out

    out = review(tmp_path, store, "RULE-BAL-TOTALS-001", "confirmed", capsys)
    assert "FM-007 probability: 0.75 -> 0.773" in out

    out = review(tmp_path, store, "RULE-LEN-001", "false_positive", capsys)
...
def test_review_duplicate_and_unknown_exit_1(tmp_path, capsys):
    ...
    run_seeded(tmp_path, store)
    capsys.readouterr()
    review(tmp_path, store, "RULE-DUP-001", "confirmed", capsys)
    fid = finding_id(tmp_path, "RULE-DUP-001")
...
def test_diff_cli(tmp_path, capsys):
    ...
    run_seeded(tmp_path, store)
    capsys.readouterr()
    review(tmp_path, store, "RULE-LOAN-BAL-001", "confirmed", capsys)
